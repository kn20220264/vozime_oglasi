...
// ─── Uplatnica popup ────────────────────────────────────────────
function BankInstructionsModal({ payment, onClose }) {
  ...
  const rows = [
    ...
    { label: 'Svrha uplate',     value: reference, mono: true, highlight: true },
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md">
        {/* Header */}
        <div className="bg-[#12142D] rounded-t-2xl px-6 py-5">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-white font-black text-lg">Plaćanje uplatnicom</h3>
              <p className="text-[#6674A3] text-xs mt-0.5">Podaci za plaćanje u banci ili pošti</p>
            </div>
            <button onClick={onClose} className="text-[#6674A3] hover:text-white transition text-xl leading-none">✕</button>
          </div>
        </div>
...
export default function PromoteModal({ adId = null, adTitle = '', packageType = 'ad_boost', onClose, onSuccess }) {
  ...
  if (step === 'bank' && paymentResult) {
    return <BankInstructionsModal payment={paymentResult} onClose={() => { onClose(); onSuccess?.(); }} />;
  }
          ...
          <div className="flex items-center justify-between">
            <div>
              <h3 className="font-black text-[#12142D] text-lg">
                {isAdBoost ? 'Promoviši oglas' : 'Nadogradi nalog'}
              </h3>
